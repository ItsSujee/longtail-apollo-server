...
export class GinAPI extends RESTDataSource { 
    ...
    async BusRoute(bs_id: string | number ) {
      return this.get(`bus_routes?bs_id=${encodeURIComponent(bs_id)}`);
    }
    ...
    async NearbyRouteCard(lat: string | number, long: string | number, n: string | number) {
      return this.get(`bus_stops?lat=${encodeURIComponent(lat)}&long=${encodeURIComponent(long)}&n=${encodeURIComponent(n)}`);
    }
}
